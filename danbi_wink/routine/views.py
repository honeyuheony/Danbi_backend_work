...
def routineList(request):
    if request.method == 'GET':
        ...
        routine = RoutineSerializer(routine, many=True).data
        res = {
            "data" : [],
            "message": {
        ...
        for r in routine:
            result = RoutineResultSerializer(Routine_result.objects.get(routine_id=r['routine_id']),
        ...
        request.data['id'] = str(request.user)
...
def routineDetail(request, routine_id):
    if request.method == 'GET':
        routine = RoutineSerializer(Routine.objects.get(routine_id=routine_id),
            context={'request': request}).data
        result = RoutineResultSerializer(Routine_result.objects.get(routine_id=routine_id),
                context={'request': request}).data
        days = RoutineDaySerializer(Routine_day.objects.get(routine_id=routine_id),
                context={'request': request}).data
        res = {
            "data" : {
                "goal" : routine['goal'],
        ...
        day = Routine_day.objects.get(routine_id=routine_id)
